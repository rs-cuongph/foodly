  ...
  const validationScheme = useMemo(
    () =>
      yup.object().shape({
        ...
        public_time_end: yup.string().label('room.public_time_end').minDate('public_time_start'),
        public_time_start: yup.string().label('room.public_time_start').when('public_time_end', {
          is: (value?: string) => !!value?.length,
          then: (schema) => schema.required()
        }),
        price: yup.number().label('room.price'),
        share_scope: yup.string().label('room.share_scope').required(),
        invited_people: yup.array().of(yup.string()).label('room.invited_people')
      }),
    []
  )
